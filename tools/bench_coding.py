#!/usr/bin/env python3
"""Coding-workload benchmark for knivesysl: cold prefill, decode@depth, multi-turn.

Builds repo-like contexts from knivesysl's own sources (code!), sizes them in
tokens, and measures via the x_knivesysl stats of serve_openai.py.
...
# prefill/decode timings are only comparable with a fixed thinking state: reasoning
# turns route through the think-overflow rescue, whose x_knivesysl.prefill_s belongs to
# the second phase. Default off (the README methodology); TQ_BENCH_THINK=1 to measure
# the reasoning path instead.
...
def chat(messages, max_tokens=192):
    ...
    wall = time.time() - t0
    return wall, out.get("x_knivesysl") or {}, out["choices"][0]["message"]["content"]
