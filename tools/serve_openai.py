#!/usr/bin/env python3
"""OpenAI-compatible API server for the knivesysl FP6 spec-decode engine.
...
(x_knivesysl: accept_len, rounds, tok/s).
...
x_knivesysl reports prefix/reused_tokens/prefilled_tokens per request.
...
ap.add_argument("--model-name", default="knivesysl-qwen3.6-27b-fp6")
...
ap.add_argument("--think-effort-caps",
                default=os.environ.get("TQ_THINK_EFFORT_CAPS", ""),
                help="OPTIONAL engine-side ceiling on top of the native tier: comma list"
                     " of TIER=CAP think tokens (0 = unlimited), e.g."
                     " low=512,medium=2048,xhigh=0. Empty (default) = upstream behaviour,"
                     " the tier's prompt instruction alone decides depth. Explicit"
                     " think_cap in the request wins; --think-cap is the fallback for"
                     " requests without an effort tier. Env: TQ_THINK_EFFORT_CAPS")
...
# Qwen3.8's chat template owns effort: reasoning_effort in ('low','medium','xhigh')
# (default xhigh) selects a system-level reasoning instruction -- 'medium' is the
# neutral prompt, 'low' asks for brief thinking, 'xhigh' asks for thorough
# validation -- and the template RAISES on any other value. So the tier is passed
# through to the template (vLLM does the same) rather than emulated server-side.
# EFFORT_ALIAS folds the wider OpenAI vocabulary onto those three; EFFORT_OFF
# turns thinking off instead. --think-effort-caps can add a hard token ceiling.
EFFORT_OFF = ("none", "minimal", "off", "0", "false", "")
TEMPLATE_EFFORTS = ("low", "medium", "xhigh")
EFFORT_ALIAS = {"low": "low", "medium": "medium", "med": "medium",
                "high": "xhigh", "xhigh": "xhigh", "max": "xhigh"}


def _parse_effort_caps(spec):
    """"low=512,xhigh=0" -> {"low": 512, "xhigh": 0}; keys fold through EFFORT_ALIAS."""
    caps = {}
    for part in str(spec).split(","):
        part = part.strip()
        if not part:
            continue
        tier, _, cap = part.partition("=")
        tier = EFFORT_ALIAS.get(tier.strip().lower(), tier.strip().lower())
        try:
            caps[tier] = max(0, int(cap))
        except ValueError:
            print(f"[serve] ignoring bad --think-effort-caps entry {part!r}", flush=True)
    return caps


EFFORT_CAPS = _parse_effort_caps(args.think_effort_caps)
...
print(f"[serve] ready on :{args.port} (eos={sorted(EOS_IDS)}, "
      ...
      f"effort={'/'.join(TEMPLATE_EFFORTS)} (default xhigh)"
      f"{' caps ' + ','.join(t + '=' + ('inf' if c == 0 else str(c)) for t, c in EFFORT_CAPS.items()) if EFFORT_CAPS else ''}, "
...
def _req_effort(req):
    """Raw reasoning effort as sent, from either endpoint's field. None = absent.
    /v1/responses sends reasoning.effort; /v1/chat/completions sends
    reasoning_effort (OpenAI/vLLM) or reasoningEffort (Vercel AI SDK)."""
    rs = req.get("reasoning")
    eff = rs.get("effort") if isinstance(rs, dict) else None
    if eff is None:
        eff = req.get("reasoning_effort", req.get("reasoningEffort"))
    return None if eff is None else str(eff).strip().lower()


def _effort_tier(req):
    """Requested effort folded onto the template's ('low','medium','xhigh').
    None = no effort field; False = an off-set value (thinking disabled)."""
    eff = _req_effort(req)
    if eff is None:
        return None
    if eff in EFFORT_OFF:
        return False
    tier = EFFORT_ALIAS.get(eff)
    if tier is None:
        print(f"[serve] reasoning_effort {eff!r} unknown -> xhigh "
              f"(template supports {'/'.join(TEMPLATE_EFFORTS)})", flush=True)
        tier = "xhigh"
    return tier


def _apply_thinking_ckw(req, ckw):
    """Resolve the template kwargs that own reasoning, precedence high -> low:
      1. top-level enable_thinking             (request is explicit)
      2. chat_template_kwargs.enable_thinking  (honored as-is)
      3. reasoning effort tier                 (off-set -> off, else on + tier)
      4. server --no-thinking default          -> off
      5. otherwise leave to the chat template  (Qwen default = on, xhigh)
    The tier itself goes to the template (it selects the reasoning instruction);
    only the on/off decision is ours. preserve_thinking rides along so Qwen-Cloud
    style top-level flags work next to chat_template_kwargs."""
    tier = _effort_tier(req)
    if "enable_thinking" in req:
        ckw["enable_thinking"] = bool(req["enable_thinking"])
    elif "enable_thinking" in ckw:
        pass
    elif tier is not None:
        ckw["enable_thinking"] = tier is not False
    elif args.no_thinking:
        ckw["enable_thinking"] = False
    if tier and ckw.get("enable_thinking", True) and "reasoning_effort" not in ckw:
        ckw["reasoning_effort"] = tier
    if "preserve_thinking" in req and "preserve_thinking" not in ckw:
        ckw["preserve_thinking"] = bool(req["preserve_thinking"])
    return ckw


def _req_think_cap(req, thinking_primed):
    """Optional engine-side think ceiling; 0 = unlimited (upstream behaviour).
    Precedence: request think_cap > --think-effort-caps tier > --think-cap."""
    if not thinking_primed:
        return 0
    if req.get("think_cap"):
        return int(req["think_cap"])
    tier = _effort_tier(req)
    if tier and tier in EFFORT_CAPS:
        return EFFORT_CAPS[tier]
    return int(args.think_cap or 0)
...
def generate(prompt_ids, max_new, temp, seed, on_tokens, no_cache=False, dbg=False,
    ...
    with ENG_LOCK:
        ...
        eos_set = () if ignore_eos else EOS_IDS      # ignore_eos: benchmark/eval fixed lengths
        if seed_tok in eos_set:
    ...
    if finish == "think_cap":
        ...
        n2, finish, st2 = generate(p2, max(1, max_new - n1), temp, seed,
                                   ...
                                   bark_decay=bd2, allow_live=True, ignore_eos=ignore_eos)
...
def responses_to_chat_inputs(req):
    ...
    if isinstance(inp, str):
        ...
        pending_reasoning = ""         # a reasoning item annotates the NEXT assistant turn
        for item in inp:
            ...
            if t == "message":
                ...
                # preserve_thinking (Qwen3.8 default ON) renders history thinking back
                # into the prompt, so keep the summary text: the reconstructed prompt
                # then extends what the engine already committed (prefix cache stays
                # live) instead of a hollow <think></think> that diverges at turn 1.
                pending_reasoning = "\n".join(
...
class H(BaseHTTPRequestHandler):
    ...
    def _do_responses(self, req):
        ...
        ckw = _apply_thinking_ckw(req, dict(req.get("chat_template_kwargs") or {}))
        try:
    ...
    def do_GET(self):
        ...
        if self.path == "/v1/models":
            return self._json(200, {"object": "list", "data": [
                {"id": args.model_name, "object": "model", "owned_by": "knivesysl"}]})
    ...
    def do_POST(self):
        ...
        # tl_debug echoes emitted/engine token ids in x_knivesysl (gate tooling)
        no_cache = bool(req.get("tl_no_cache", False))
        dbg = bool(req.get("tl_debug", False))
